...
<template>
  <a :href="item.to" :class="[!item.available ? 'opacity-60 cursor-not-allowed pointer-events-none' : '', 'w-full border-solid border border-layoutBorder rounded-lg divide-y divide-layoutBorder hover:shadow']">
    ...
    <div>
      <div class="-mt-px flex divide-x divide-layoutBorder">
        <div class="w-0 flex-1 flex">
          <div class="relative -mr-px w-0 flex-1 inline-flex py-4 px-6 text-sm text-gray-700 font-medium rounded-bl-lg hover:text-gray-500">
            <span>{{ item.name }}</span>
          </div>
        </div>
